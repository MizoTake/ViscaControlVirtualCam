...
- `ipSetupNetwork.logicalMask` / `ipSetupNetwork.logicalGateway` は `ENQ:network` 応答の `MASK` / `GATEWAY` に使用
- `ipSetupNetwork.logicalIp` は広告IPが解決不能な場合のフォールバックとして使用
