...
public class ViscaServerCoreTests
{
    ...
    [Test]
    public void ViscaIp_UnsupportedPayloadType_ReturnsNotExecutable()
    {
        var handler = new StubHandler();
        var server = new ViscaServerCore(handler, new ViscaServerOptions
        {
        ...
        byte[] payload = { 0x82, 0x01, 0x04, 0x3F, 0x02, 0x00, 0xFF };
        var packet = BuildViscaIpPacket(0x01, 0x22, (ushort)payload.Length, 0x00000003u, payload);
        InvokeProcessFrame(server, packet, sent);
        server.Dispose();

        Assert.AreEqual(1, sent.Count, "Should respond once");
        var resp = sent[0];
        CollectionAssert.AreEqual(new byte[] { 0x01, 0x11, 0x00, 0x04, 0x00, 0x00, 0x00, 0x03 },
            resp.Take(8).ToArray());
        CollectionAssert.AreEqual(new byte[] { 0x90, 0x62, ViscaProtocol.ErrorCommandNotExecutable, 0xFF },
            resp.Skip(8).ToArray());
    }

    [Test]
    public void ViscaIp_PayloadLengthMismatch_ReturnsMessageLengthError()
    {
        var handler = new StubHandler();
        var server = new ViscaServerCore(handler, new ViscaServerOptions
        {
        ...
        byte[] payload = { 0x81, 0x21, 0xFF }; // actual length=3
        var packet = BuildViscaIpPacket(0x01, 0x00, 0x0002, 0x00000004u, payload); // declared=2
        InvokeProcessFrame(server, packet, sent);
        server.Dispose();

        Assert.AreEqual(1, sent.Count, "Should respond once");
        var resp = sent[0];
        CollectionAssert.AreEqual(new byte[] { 0x01, 0x11, 0x00, 0x04, 0x00, 0x00, 0x00, 0x04 },
            resp.Take(8).ToArray());
        CollectionAssert.AreEqual(new byte[] { 0x90, 0x61, ViscaProtocol.ErrorMessageLength, 0xFF },
            resp.Skip(8).ToArray());
    }

    [Test]
    public void ViscaIp_ReplyPayloadType_IsAcceptedAndHandled()
    {
        var handler = new StubHandler();
        var server = new ViscaServerCore(handler, new ViscaServerOptions
        {
        ...
        byte[] payload = { 0x83, 0x01, 0x04, 0x3F, 0x02, 0x00, 0xFF };
        var packet = BuildViscaIpPacket(0x01, 0x11, (ushort)payload.Length, 0x00000005u, payload);
        InvokeProcessFrame(server, packet, sent);
        server.Dispose();

        Assert.AreEqual(2, sent.Count, "StubHandler should send ACK and Completion");
        foreach (var resp in sent)
        {
            Assert.AreEqual(0x01, resp[0]);
            Assert.AreEqual(0x11, resp[1]);
            CollectionAssert.AreEqual(new byte[] { 0x00, 0x00, 0x00, 0x05 }, resp.Skip(4).Take(4).ToArray());
        }
    }
