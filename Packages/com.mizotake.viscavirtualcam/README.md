...
- **Inquiry Polling Sync**: 実機への Inquiry (`8X 09 06 12 FF` / `8X 09 04 47 FF`) を定期送信して、Unity 側 PTZ を追従
- **Blackmagic PTZ Control**: ATEM スイッチャー向け拡張サポート
- **Sony RM-IP500 互換初期化**: Interface Clear / Power / Version Inquiry に対応
...
- **System/Camera**: IF_Clear、Camera Power On/Off、Power Inquiry、Version Inquiry
...
- **Inquiry Polling (Real Camera -> Virtual Sync)**:
  - `Enable Inquiry Polling`: 実機への問い合わせを有効化
  - `Inquiry Polling Interval Ms`: 問い合わせ周期（既定 `100ms`）
  - `Inquiry Timeout Ms`: 応答待ちタイムアウト（既定 `150ms`）
  - `Inquiry Retry Count`: タイムアウト時の再送回数（同一シーケンス番号で再送）
  - `Apply Inquiry To Virtual Camera`: 取得値を Unity の PTZ リグへ反映
...
- Sony RM-IP500 からの初期化シーケンス（IF_Clear / Power / Version Inquiry）にも対応
